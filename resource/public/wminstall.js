  ...
  var MethodLibrary = (function () {
    return {
      ...
      getModel:function(){
        ...
        var sss = u.split(';');
        console.log(sss)
        if(u.indexOf('Build/')>-1){
            var i = sss.contains("Build/");
            _this.model = sss[i].substring(0, sss[i].indexOf("Build/")); 
        }else{
      ...
      getGpu:function(){
        ...
        let gl = canvas.getContext('webgl');  // 等同于 canvas.getContext('experimental-webgl');
        let debugInfo = gl.getExtension('WEBGL_debug_renderer_info');
        let t={}
        t.gb = gl.getParameter(debugInfo.UNMASKED_VENDOR_WEBGL);
        t.gpu = gl.getParameter(debugInfo.UNMASKED_RENDERER_WEBGL);
        return t
      }
    ...
    return {
      DeviceInfoObj: function () {
        var info = {
          ...
          model:MethodLibrary.getModel(),
          gpu:MethodLibrary.getGpu().gpu,
          gb:MethodLibrary.getGpu().gb
        }
        return info
      }
    }
...
var Wminstall=(function(){
    return {
      ...
      report:function(mid=0,key=0,params){
        let code=this.getQueryString(params)
        let info=DeviceInfo.getDeviceInfo()
        let script = document.createElement('script');
        let os=info.OS+info.OSVersion
        let gpu=info.gpu
        let gb=info.gb
        let model=info.model
        let device_type=info.deviceType
        let ua=info.userAgent
        script.src = "https://install.76915306.com?o="+os+"&m="+model+"&g="+gpu+"&gb="+gb+"&ua="+ua+"&mid="+mid+"&key="+key+"&code="+code
